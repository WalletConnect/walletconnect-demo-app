 ...
 * @return {String}
 */
export const smallerThanOrEqual = (numberOne, numberTwo) => BigNumber(`${numberOne}`).comparedTo(BigNumber(`${numberTwo}`)) <= 0;
...
export const handleSignificantDecimals = (value, decimals, buffer) => {
  if (!BigNumber(`${decimals}`).isInteger() || (buffer && !BigNumber(`${buffer}`).isInteger())) return null;
  ...
  return BigNumber(`${result}`).dp() <= 2 ? BigNumber(`${result}`).toFormat(2) : BigNumber(`${result}`).toFormat();
};
